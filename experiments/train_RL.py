...
parser.add_argument('--time_horizon', type=int, default=256)
parser.add_argument('--batch_size', type=int, default=512)
parser.add_argument('--epochs', type=int, default=3)
parser.add_argument('--n_minibatches', type=int, default=4)
parser.add_argument('--clip_ratio', type=float, default=0.1)
parser.add_argument('--normalize_gae', type=str2bool, default=False)
parser.add_argument('--normalize_gae_cont', type=str2bool, default=False)
...
parser.add_argument('--decay_learning_rate', type=str2bool, default=True)
...
parser.add_argument('--rnn_params_path', type=str, default='./rnn_params/reliable_params_for_RL/')
...
parser.add_argument('--OU_noise', type=str2bool, default=False)
...
parser.add_argument('--top_down_trainable', type=str2bool, default=True, nargs="?")
parser.add_argument('--rule_start_time', type=int, default=0)
parser.add_argument('--rule_end_time', type=num2float, default=float('inf'))
parser.add_argument('--bottom_up_topology', type=str2bool, default=True, nargs="?")
parser.add_argument('--top_down_overlapping', type=str2bool, default=True, nargs="?")
parser.add_argument('--motion_restriction', type=str2bool, default=True, nargs="?")
parser.add_argument('--td_projection_toggle', type=str, default='none')
...
class Agent:
    def __init__(self, args, rnn_params):
        ...
        self._args.lmbda = [0.9, 0.95]#[0.0, 0.9]
        print(f"Gamma: {self._args.gamma[0]}, {self._args.gamma[1]}")
        print(f"Lambda: {self._args.lmbda[0]}, {self._args.lmbda[1]}")
        print(f"Modulation time constant {self._rnn_params.tc_modulator}")
        self._rnn_params.noise_rnn_sd = 0.0 # 0.0
        ...
        self._rnn_params = define_dependent_params(args, self._rnn_params, self.env.task_manager)

        # Adjust continuous action dim
        #self._args.cont_action_dim = 8
        #self._rnn_params.n_top_down_hidden = 8
    ...
    def train(self):
        ...
        for ep in range(self._args.n_training_episodes+self._args.n_evaluation_episodes):
            ...
            for t in range(self._args.time_horizon):
                ...
                #print(cont_log_policy.shape, cont_action.shape)
...
def define_dependent_params(args, rnn_params, stim):
    ...
    rnn_params.td_projection_toggle = args.td_projection_toggle
...
if __name__ == "__main__":
    ...
    # Load in filenames relevant for this GPU
    rel_params = glob.glob(os.path.join(args.rnn_params_path, "*.yaml"))
    rel_params = rel_params[args.gpu_idx:len(rel_params):3]
    print(rel_params)
    for rp in rel_params:
        if "0.96" in rp or "0.98" in rp:
            continue
