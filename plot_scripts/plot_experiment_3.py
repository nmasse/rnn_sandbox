...
def plot_results(data_dir, base_dir = args.base_dir):
    ...
    filt_acc_last_few = np.mean(filtered_accs[-10:,:], axis=0)[:,np.newaxis]


    # Make df
    acc_flat = filtered_accs.flatten()
    alphastr = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    nets = np.tile(np.repeat(np.arange(9), 5), filtered_accs.shape[0]).astype(np.int16)
    ...
    alphastr = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    nets_last = np.repeat(np.arange(9), 5).astype(np.int16)
    ...
    fig.tight_layout()
    fig.savefig("/Users/mattrosen/results_figs/exp3_acc.png", dpi=500)
    plt.close(fig)
    ...
    ax.set(title='Bio-constrained learning accuracy', ylim=[0.5, 1.0])
    fig.tight_layout()
    fig.savefig("/Users/mattrosen/results_figs/exp3_reliability_singlesubplot.png", dpi=500)
    plt.close(fig)
