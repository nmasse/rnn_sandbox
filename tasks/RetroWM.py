...
class RetroWM(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        """
        Generate RetroWM trials (n_RFs stimuli)
        Modeled off specifically retrospective trials from the task 
        in the Buschman paper, but with the allowance for k different 
        sample stimuli to be presented at once.
        """

        # Set up trial_info with RetroWM-specific entries, and amending the entry for sample
        ...
        for i in range(batch_size):
            ...
            cue_bounds = [mid_delay, delay_bounds[-1]]#self.trial_length]
