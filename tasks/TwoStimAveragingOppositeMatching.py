...
class TwoStimAveragingOppositeMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            average_sample_opp = (average_sample + np.pi) % (2 * np.pi)
            resp_idx = int(average_sample_opp // (2 * np.pi / (self.n_output - 1)) + 1)
