...
class TwoStimMinIndicationMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            # Generate outputs
            min_sample = np.amin(sample_dirs) / self.n_motion_dirs * 2 * np.pi
            min_sample = min_sample % (2 * np.pi)
            resp_idx = int(min_sample // (2 * np.pi / (self.n_output - 1)) + 1)
