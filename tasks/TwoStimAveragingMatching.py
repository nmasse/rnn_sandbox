...
class TwoStimAveragingMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            # Generate outputs
            average_sample = np.mean(sample_dirs) / self.n_motion_dirs * 2 * np.pi
            average_sample = average_sample % (2 * np.pi)
            resp_idx = int(average_sample // (2 * np.pi / (self.n_output - 1)) + 1)
            ...
            if match:
                ...
                test_dir = np.random.choice(
                    np.setdiff1d(np.arange(self.n_motion_dirs), resp_idx - 1))
