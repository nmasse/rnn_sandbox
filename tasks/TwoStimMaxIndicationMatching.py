...
class TwoStimMaxIndicationMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            # Generate outputs
            max_sample = np.amax(sample_dirs) / self.n_motion_dirs * 2 * np.pi
            max_sample = max_sample % (2 * np.pi)
            resp_idx = int(max_sample // (2 * np.pi / (self.n_output - 1)) + 1)
