...
class TwoStimSubtractingRightLeftMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            # Generate outputs
            dif_sample_rl = (sample_dirs[1] - sample_dirs[0]) / self.n_motion_dirs * 2 * np.pi
            dif_sample_rl = dif_sample_rl % (2 * np.pi)
            resp_idx = int(dif_sample_rl // (2 * np.pi / (self.n_output - 1)) + 1)
