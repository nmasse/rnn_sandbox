...
class TwoStimRightIndicationOppositeMatching(Task.Task):
    ...
    def generate_trials(self, batch_size, test_mode=False):
        ...
        for i in range(batch_size):
            ...
            # Generate outputs
            right_sample = sample_dirs[1] / self.n_motion_dirs * 2 * np.pi
            right_sample_opp = (right_sample + np.pi) % (2 * np.pi)
            resp_idx = int(right_sample_opp // (2 * np.pi / (self.n_output - 1)) + 1)
